...
void LCD_Scan_DIR(u8 dire)
{
      ...
      else  //if((LCD_Manage.ID == 0x9320) || (LCD_Manage.ID == 0x9325) || (LCD_Manage.ID == 0x9328))
      {
            switch(dire)
            {                //            I/D1       I/D0       AM
                  case L2R_U2D:
...
u16 LCD_ReadPointColor(u16 x,u16 y)
{
	...
	if(LCD_Manage.ID == 0x9341)	LCD_Write_Register(0x2e);	//9341发送读GRAM指令
	else	LCD_Write_Register(0x22);	//其他芯片

	LCD_Read_Data();	//空读一次
	LCD_Delay(2);
	r = LCD_Read_Data();	//读取颜色值

	if(LCD_Manage.ID == 0x9341)	//这个奇葩要读取两次，晕
	{
		LCD_Delay(2);	//等待屏幕发送数据到总线
		b = LCD_Read_Data();	//接收最后一组数据
...
//		LCD_Write_Register(0x2c);	//GRAM开始写指令
	...
	else if((LCD_Manage.ID == 0x9320) || (LCD_Manage.ID == 0x9325) || (LCD_Manage.ID == 0x9328))
	{
		...
		LCD_WriteRegisterData(0x0003,0x1030);	//设置LCD的RGB格式，显示方向等参数，65536色，565格式 
		...
		LCD_WriteRegisterData(0x000C,0x0000);	//设置RGB数据宽度 
		LCD_WriteRegisterData(0x000D,0x0000);
		LCD_WriteRegisterData(0x000F,0x0000);

		//power on sequence VGHVGL
		LCD_WriteRegisterData(0x0010,0x0000);	//退出睡眠模式、待机模式   
		...
		//区域操作
		LCD_WriteRegisterData(0x0080,0x0000);	//Partial Image 1 Display Position 
		LCD_WriteRegisterData(0x0081,0x0000);	//Partial Image 1 RAM Start/End Address 
		LCD_WriteRegisterData(0x0082,0x0000); 
		LCD_WriteRegisterData(0x0083,0x0000); 	//Partial Image 2 Display Position
		LCD_WriteRegisterData(0x0084,0x0000);	//Partial Image 2 RAM Start/End Address 
		...
		//
		LCD_WriteRegisterData(0x0007,0x0133);	//控制LCD开启/关闭显示
	}
