...
RESULT Joystick_Data_Setup(uint8_t RequestNo)
{
  ...
  CopyRoutine = NULL;
		...
		{
			if((pInformation->USBwValue==((0x0002<<8)|0x03))
						&& (pInformation->USBwIndex == 0)&&(pInformation->USBwLength==2))         //get feature of MTouch report
			{
				CopyRoutine = Joystick_GetReport;
			}
		}else if((pInformation->USBbmRequestType ==0x21)
					&& RequestNo == SET_REPORT)
	{
		if((pInformation->USBwValue==((0x0003<<8)|0x03))
					&& (pInformation->USBwIndex == 0)&&(pInformation->USBwLength==3))
		{
...
/*******************************************************************************/
uint8_t MaxContactCountReport[2]=
{
	0x02,                         /*REPORTID_MTOUCH*/
	MAX_TOUCH_POINT,              /*MAX_TOUCH_POINT*/
};
/*******************************************************************************/
uint8_t *Joystick_GetReport(uint16_t Length)
{
  ...
  return (uint8_t *)(&MaxContactCountReport);
}
/******************************************************************************/


uint8_t *Joystick_SetReport(uint16_t Length)
{
	if (Length == 0)
  {
    pInformation->Ctrl_Info.Usb_wLength = sizeof(TouchMode);
    return 0;
  }
  return (uint8_t *)(&TouchMode);
}
